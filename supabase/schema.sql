...
-- We use a SECURITY DEFINER function to check membership to avoid RLS recursion
-- Create helper function first (defined in fix-projects-policy.sql)
...
CREATE POLICY "Users can view own or member projects"
  ...
  USING (
    ...
    owner_id = auth.uid()
    OR
    -- Check membership via SECURITY DEFINER function (bypasses RLS recursion)
...
-- Users can read scans if they own or are members of the project
...
-- Users can insert scans if they own or are members of the project
...
-- Users can delete scans if they own or are members of the project
